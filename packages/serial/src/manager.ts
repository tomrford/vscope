...
export const summarizeDevice = Effect.fn("VScopeSerial.summarizeDevice")(function* (
  device: VScopeDevice,
) {
...
export class VScopeSerial extends Context.Service<VScopeSerial, VScopeSerialService>()(
  "@vscope/serial/VScopeSerial",
) {}

export const makeVScopeSerial = Effect.fn("VScopeSerial.make")(function* (
  options: VScopeSerialOptions = {},
): Effect.fn.Return<VScopeSerialService, never, Scope.Scope> {
  ...
  const closeEntry = Effect.fn("VScopeSerial.closeEntry")(function* (
    path: string,
    entry: DeviceEntry,
  ) {
  ...
  const closeManagedDevice = Effect.fn("VScopeSerial.closeManagedDevice")(function* (
    path: string,
    token: symbol,
  ) {
  ...
  const publishLostDevice = Effect.fn("VScopeSerial.publishLostDevice")(function* (
    path: string,
    token: symbol,
    cause: VScopeDeviceError,
  ) {
    return yield* lock.withPermit(
      Effect.gen(function* () {
        const entries = yield* Ref.get(entriesRef);
...
export const makeVScopeSerialLayer = (options: VScopeSerialOptions): Layer.Layer<VScopeSerial> =>
  Layer.effect(VScopeSerial, makeVScopeSerial(options));
